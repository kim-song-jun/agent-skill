...
// Real adversarialVerify behavior tests against the COPILOT-vendored module.
// Proves the vendored module + copilot registry.mjs wiring actually runs, not
// just exists. Zero fake/shape-only assertions.
// NOTE: the pure-JS module IS real behavior (tests here). The *live Copilot
// `task` dispatch* of the adversarial step is spec-level and live-CLI-unverified
// (#27) until the Copilot CLI spike. These tests assert module-level contracts only.
...
// Import from the COPILOT-vendored path — this is the key proof that the vendor
// landed and the copilot registry.mjs wiring resolves correctly.
import { adversarialVerify } from "../../../plugins/harness-floor-copilot/skills/agent-all-copilot/lib/verification-adapters/adversarial-verifier.mjs";

function fixtureDir(seed) {
  return mkdtempSync(resolve(tmpdir(), `copilot-adversarial-${seed}-`));
}

test("copilot-vendored adversarialVerify: failing _runner → VERIFICATION_AUDIT: failed with correct evidence", async () => {
  ...
  assert.equal(result.audit, "VERIFICATION_AUDIT: failed",
    "copilot adversarialVerify must produce VERIFICATION_AUDIT: failed on exitCode 1");
  assert.equal(result.exitCode, 1);
  assert.equal(result.evidence.schemaVersion, "verification-evidence/v1");
  assert.equal(result.evidence.status, "failed");
});

test("copilot-vendored adversarialVerify: passing _runner → VERIFICATION_AUDIT: passed", async () => {
...
test("copilot-vendored adversarialVerify: spurious implementerOutput key is ignored — independence is structural", async () => {
...
test("copilot-vendored adversarialVerify: structural signature guard — no implementer self-report param", () => {
  assert.ok(
...
test("copilot-vendored adversarialVerify: real defaultCommandRunner spawns real child process (no _runner injection)", async () => {
...
test("copilot-vendored adversarialVerify: evidence conforms to verification-evidence/v1 for both pass and fail", async () => {
