...
// Real adversarialVerify behavior tests against the CODEX-vendored module.
// Proves the vendored module + codex registry.mjs wiring actually runs, not
...
// Import from the CODEX-vendored path — this is the key proof that the vendor
// landed and the codex registry.mjs wiring resolves correctly.
import { adversarialVerify } from "../../plugins/harness-floor-codex/skills/agent-all-codex/lib/verification-adapters/adversarial-verifier.mjs";

function fixtureDir(seed) {
  return mkdtempSync(resolve(tmpdir(), `codex-adversarial-${seed}-`));
}

test("codex-vendored adversarialVerify: failing _runner → VERIFICATION_AUDIT: failed with correct evidence", async () => {
  ...
  assert.equal(result.audit, "VERIFICATION_AUDIT: failed",
    "codex adversarialVerify must produce VERIFICATION_AUDIT: failed on exitCode 1");
  assert.equal(result.exitCode, 1);
  assert.equal(result.evidence.schemaVersion, "verification-evidence/v1");
  assert.equal(result.evidence.status, "failed");
});

test("codex-vendored adversarialVerify: passing _runner → VERIFICATION_AUDIT: passed", async () => {
...
test("codex-vendored adversarialVerify: spurious implementerOutput key is ignored — independence is structural", async () => {
...
test("codex-vendored adversarialVerify: structural signature guard — no implementer self-report param", () => {
  assert.ok(
...
test("codex-vendored adversarialVerify: real defaultCommandRunner spawns real child process (no _runner injection)", async () => {
...
test("codex-vendored adversarialVerify: evidence conforms to verification-evidence/v1 for both pass and fail", async () => {
