...
test("readme files describe the current Codex config surface and current test count", () => {
  for (const path of ["README.md", "README.ko.md"]) {
    ...
    assert.match(body, /harness-builder[\s\S]{0,120}v0\.6\.4/i);
    assert.match(body, /harness-floor[\s\S]{0,120}v0\.6\.4/i);
    assert.match(body, /(other 17 installable|나머지 설치 가능한)[\s\S]{0,180}v0\.6\.4/i);
...
test("operational hardening docs record implemented release-audited status", () => {
  ...
  assert.match(plan, /node --test[\s\S]{0,120}2017\/2017/);
  assert.match(plan, /release-smoke\.sh --fast --with-live-cli[\s\S]{0,120}505\/505/);
  ...
  assert.match(changelog, /2017\/2017 passing/);
  assert.match(changelog, /505\/505 passing/);
  ...
  assert.match(changelogKo, /2017\/2017 통과/);
  assert.match(changelogKo, /505\/505 통과/);
