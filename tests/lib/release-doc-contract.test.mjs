...
test("readme files describe the current Codex config surface and current test count", () => {
  for (const path of ["README.md", "README.ko.md"]) {
    ...
    assert.match(
      body,
      /(?:scripts\/doctor\.mjs[\s\S]{0,360}--platform=claude\|codex|doctor[\s\S]{0,360}Claude\/Codex scaffold)/i,
    );
    assert.match(body, /doctor[\s\S]{0,260}(operational, builder, or lite|operational\/builder\/lite)/i);
    assert.match(body, /install-platform\.sh[\s\S]{0,360}--platform=codex[\s\S]{0,180}--lite/);
...
test("readme files describe release-safe update and per-platform install paths", () => {
  ...
  assert.match(english, /Codex `all`, `builder`, and `--lite` installs run the post-install doctor automatically/i);
  assert.match(english, /--no-doctor[\s\S]{0,120}deferring validation/i);
  assert.match(english, /doctor\.mjs[\s\S]{0,360}project-local scaffold[\s\S]{0,260}non-zero exit/i);
  ...
  assert.match(korean, /Codex `all`, `builder`, `--lite` 설치는 post-install doctor를 자동 실행/i);
  assert.match(korean, /검증을 의도적으로 미룰[\s\S]{0,160}--no-doctor/i);
  assert.match(korean, /doctor\.mjs[\s\S]{0,420}project-local scaffold[\s\S]{0,320}non-zero exit/i);
...
test("operational hardening docs record implemented release-audited status", () => {
  ...
  assert.match(plan, /node --test[\s\S]{0,120}1697\/1697/);
  assert.match(plan, /release-smoke\.sh --fast --with-live-cli[\s\S]{0,120}339\/339/);
