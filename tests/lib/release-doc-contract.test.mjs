...
test("readme files describe the current Codex config surface and current test count", () => {
  for (const path of ["README.md", "README.ko.md"]) {
    const body = read(path);
    assert.match(body, /2205\/2205/);
    assert.doesNotMatch(body, /2202\/2202|2202%20passing|2202 tests|2151\/2151|2151%20passing|2151 tests|2150\/2150|2150%20passing|2150 tests|2037\/2037|2037%20passing|2037 tests|2026\/2026|2026%20passing|2026 tests|2011\/2011|2011%20passing|2011 tests|1991\/1991|1991%20passing|1991 tests/);
    ...
    assert.match(body, /harness-builder[\s\S]{0,120}v0\.7\.1/i);
    assert.match(body, /harness-floor[\s\S]{0,120}v0\.7\.1/i);
    assert.match(body, /(other 17 installable|나머지 설치 가능한)[\s\S]{0,180}v0\.7\.1/i);
