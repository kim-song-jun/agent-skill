...
test("readme files describe the current Codex config surface and current test count", () => {
  for (const path of ["README.md", "README.ko.md"]) {
    const body = read(path);
    assert.match(body, /1726\/1726/);
    assert.doesNotMatch(body, /1721\/1721|1721%20passing|1721 tests/);
...
test("operational hardening docs record implemented release-audited status", () => {
  ...
  assert.match(plan, /node --test[\s\S]{0,120}1726\/1726/);
  assert.match(plan, /release-smoke\.sh --fast --with-live-cli[\s\S]{0,120}372\/372/);
