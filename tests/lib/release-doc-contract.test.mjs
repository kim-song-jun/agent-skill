...
test("Codex runtime specs describe the current sequential surface instead of stale agent hooks", () => {
  ...
  for (const path of [
    ...
    assert.doesNotMatch(body, /1719\/1719/);
    assert.match(body, /1721\/1721/);
...
test("operational hardening docs record implemented release-audited status", () => {
  ...
  assert.match(plan, /node --test[\s\S]{0,120}1721\/1721/);
  assert.match(plan, /release-smoke\.sh --fast --with-live-cli[\s\S]{0,120}367\/367/);
