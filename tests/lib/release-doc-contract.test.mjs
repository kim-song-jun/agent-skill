...
test("readme files describe the current Codex config surface and current test count", () => {
  for (const path of ["README.md", "README.ko.md"]) {
    ...
    assert.doesNotMatch(body, /2017\/2017|2017%20passing|2017 tests/);
    ...
    assert.match(body, /harness-builder[\s\S]{0,120}v0\.6\.5/i);
    assert.match(body, /harness-floor[\s\S]{0,120}v0\.6\.5/i);
    assert.match(body, /(other 17 installable|나머지 설치 가능한)[\s\S]{0,180}v0\.6\.5/i);
...
test("Codex runtime specs describe the current sequential surface instead of stale agent hooks", () => {
  ...
  for (const path of [
    ...
    assert.match(body, /1991\/1991/);
    assert.doesNotMatch(body, /2017\/2017/);
