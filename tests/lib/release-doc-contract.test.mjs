...
test("readme files describe the current Codex config surface and current test count", () => {
  for (const path of ["README.md", "README.ko.md"]) {
    const body = read(path);
    assert.match(body, /1564\/1564/);
    assert.doesNotMatch(body, /1279\/1279|1279\+|1279%20passing/);
    assert.doesNotMatch(
      body,
      /1552\/1552|1552%20passing|1554\/1554|1554%20passing|1557\/1557|1557%20passing|1558\/1558|1558%20passing|1559\/1559|1559%20passing|1560\/1560|1560%20passing|1561\/1561|1561%20passing/,
    );
    assert.doesNotMatch(body, /\[\[hooks\.agent\]\]/);
