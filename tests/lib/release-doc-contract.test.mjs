...
test("Codex runtime specs describe the current sequential surface instead of stale agent hooks", () => {
  ...
  for (const path of [
    ...
    const body = read(path);
    assert.doesNotMatch(body, /\[\[hooks\.agent\]\]/);
    assert.doesNotMatch(body, /1246\/1246|1279\/1279|1645\/1645|1646\/1646|1647\/1647|1648\/1648|1649\/1649|1650\/1650|1651\/1651/);
    assert.match(body, /1652\/1652/);
...
test("operational hardening docs record implemented release-audited status", () => {
  ...
  assert.match(plan, /node --test[\s\S]{0,120}1652\/1652/);
  assert.match(plan, /release-smoke\.sh --fast --with-live-cli[\s\S]{0,120}297\/297/);
