...
test("readme files describe the current Codex config surface and current test count", () => {
  for (const path of ["README.md", "README.ko.md"]) {
    ...
    assert.match(body, /harness-builder[\s\S]{0,120}v0\.7\.5/i);
    assert.match(body, /harness-floor[\s\S]{0,120}v0\.7\.5/i);
    assert.match(body, /(other 17 installable|나머지 설치 가능한)[\s\S]{0,180}v0\.7\.5/i);
