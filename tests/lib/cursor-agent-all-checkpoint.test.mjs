// tests/lib/cursor-agent-all-checkpoint.test.mjs
// Real flushCheckpoint/recallLatestCheckpoint round-trip tests against the
// CURSOR-vendored memory-agent + cursor-LOCAL memory-bridge. The import paths
// here assert that the vendor landed and the LOCAL cursor bridge resolves.
...
// mid-wave-death + --resume on a running Cursor background agent is spec-level
// and live-CLI-unverified (#27) until the Cursor spike.
...
// Import from cursor-LOCAL memory-bridge — asserts the vendor landed and the
// local bridge resolves. If the CC cross-plugin path were vendored verbatim
// this import would fail with ERR_MODULE_NOT_FOUND.
import { makeFileMirror } from "../../plugins/harness-floor-cursor/skills/agent-all-cursor/lib/memory-bridge.mjs";

// Import from cursor-vendored memory-agent (which must resolve ./memory-bridge.mjs locally).
import { flushCheckpoint, recallLatestCheckpoint } from "../../plugins/harness-floor-cursor/skills/agent-all-cursor/lib/memory-agent.mjs";

function freshEnv() {
  const cwd = mkdtempSync(join(tmpdir(), "cursor-memory-chk-"));
...
test("cursor-checkpoint: flushCheckpoint writes BOTH history key AND LATEST pointer; returns recoverable:true", async () => {
...
test("cursor-checkpoint: JSONL entry has schemaVersion memory-log/v1 and correct fields", async () => {
  const { cwd, fileMirror } = freshEnv();
  await flushCheckpoint({
    cwd,
    runId: "run-cursor-schema",
    wave: 1,
    iter: 2,
    phase: "3a",
    inFlight: true,
  ...
  const logPath = join(cwd, ".agent-skill/runs/run-cursor-schema/memory-log.jsonl");
...
// This is the real Phase-0 step-5b resume path with no handoff context.
test("cursor-checkpoint: genuine mid-wave death round-trip reconstructs in-flight scoping FROM DISK", async () => {
  // === HALF 1: WRITE PHASE (simulate alive coordinator reaching step 3a.0) ===
  const { cwd, fileMirror } = freshEnv();
...
test("cursor-checkpoint: flush without fileMirror returns ok:false, recoverable:false", async () => {
  const { cwd } = freshEnv();
...
test("cursor-checkpoint: recallLatestCheckpoint on pristine cwd returns found:false", async () => {
  const emptyCwd = mkdtempSync(join(tmpdir(), "cursor-memory-empty-"));
