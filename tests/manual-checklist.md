...
- Codex install renderers for operational, lite, and debug profiles, including `/codex-init --lite` via `install-platform.sh --platform=codex --lite` and debug-only installs via `--theme=debug`.
...
- Fresh release fixture coverage for Claude marketplace dry-run, Claude operational/lite render output, Claude terminal `install-platform.sh --platform=claude` operational/lite installs, and Codex operational/lite/debug installs in new git repos.
