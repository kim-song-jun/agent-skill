...
- Latest hardening addition: Codex command-surface release coverage for `codex-init --help` and unknown-flag handling, alongside `scripts/release-fixture-smoke.mjs`, a fresh fixture smoke gate covering Claude marketplace dry-run, Claude operational render output, and Codex operational/lite installs in new git repos. It complements `scripts/release-audit.mjs`, the Claude/Codex release readiness matrix for marketplace manifests, required init/floor/thrift files, hook schema expectations, reviewer routing, audit tokens, and root role routing.
...
- `node scripts/release-fixture-smoke.mjs`: Claude marketplace dry-run, Claude operational render output, and Codex operational/lite fresh fixtures passing.
