...
- `node scripts/release-audit.mjs`: Claude 55/55 and Codex 61/61 readiness checks passing, including the final `scripts/release-smoke.sh` gate contract, doctor recovery guidance, Codex floor-conditional language guidance, Claude companion root guidance, Codex thrift advisory summariser contract, the user objective release matrix, and public CLI executable/shebang packaging.
