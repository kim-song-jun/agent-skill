...
`harness-debug-codex` deliberately does NOT re-implement the prompt
engineering inside `superpowers:systematic-debugging`. Instead it
**wraps and persists** that skill. Both can be installed
side-by-side; `harness-debug-codex` becomes the orchestrator and the
...
2. **Phase 3 (Hypothesize)** auto-loads the skill when the supervisor
   field is populated and Codex exposes installed skill invocation:
   - Renders `templates/hypothesis-prompt.md.hbs` with the structured
     failure and prior rejections.
   - Invokes `superpowers:systematic-debugging`, passing the rendered
     prompt as the initial input.
...
3. **State persistence** is the key value-add. The skill's transient
   ...
   turn (or the next session via `run /debug --resume`) starts from structured
...
4. **Phase 4 honours the skill's Phase 4 (Implementation).** When the
   superpowers skill advances to proposing an experiment, harness-debug-codex
   runs that experiment instead of inventing its own. The skill
   remains the source of truth for HOW to think; harness-debug-codex
   supplies WHAT to think about and WHERE the conclusions land.

## Graceful fallback

When `superpowers:systematic-debugging` is not installed,
or when the current Codex host does not expose skill invocation,
`harness-debug-codex` uses the inlined version of the same template
...
mismatches are detectable; later iterations may pin a known-good
...
```js
// Pseudocode — actual probe depends on the current Codex skill surface.
