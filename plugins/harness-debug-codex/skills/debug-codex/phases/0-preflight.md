...
6. If `superpowers:systematic-debugging` is installed (probe the active
   Codex skill list or plugin cache when the host exposes one), record
   `state.supervisor = {wrappedSkill: "superpowers:systematic-debugging"}`
   for Phase 3. If the probe is unavailable or errors, record fallback
   mode and keep going.
