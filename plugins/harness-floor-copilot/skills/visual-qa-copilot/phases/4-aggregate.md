...
1. Walk `<slug-dir>/**/<image>.analysis.json` via `bash("find ...")`
   plus `view` per file. Merge into `report.json` keyed by
   `{page, breakpoint, component, state}`.
2. If `priorRunDir`: `view("<priorRunDir>/report.json")`. Diff per-issue.
...
4. Render `templates/report.md.hbs`. Pass it through
   ...
   redacted content to `<slug-dir>/report.md` via `create` / `edit`.
5. Render `<slug-dir>/report.json`, pass it through the same redaction gate,
   and write via `create` / `edit` only if high-severity findings did not block it.
