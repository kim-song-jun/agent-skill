...
3. If slug dir exists AND `--force`: `bash("rm -rf <slugDir>")`.
4. If slug dir exists AND no `--force`: abort with retry hint.
5. Find prior run: `bash("ls -td <config.output.dir>/*/ | grep -v <slug> | head -1")`.
   Stash `priorRunDir` (or null).
6. Create slug dir + per-page subdirs via `bash("mkdir -p ...")`.
