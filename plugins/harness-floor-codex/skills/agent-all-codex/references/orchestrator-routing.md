# Orchestrator routing: `/agent-all-codex` vs a Codex research sweep
...
|---|---|
| A durable code change shipped as a PR, gated by model-judged reviewers (VERIFICATION_AUDIT / QA_AUDIT / ORCHESTRATION_AUDIT tokens) | **`/agent-all-codex`** |
...
| Mixed (research **then** ship) | **`codex exec` sweep first** → it writes a `validateTaskDoc`-compliant task doc → then **`/agent-all-codex <taskdoc> --no-brainstorm`** consumes it |

Rule of thumb: if the output is **files committed behind gates**, that is
`/agent-all-codex`. If the output is **findings / specs / answers**, that is a
...
`/agent-all-codex` even when you might be tempted to use a sweep.

## They are siblings — never nest them

A `codex exec` research sweep has no awareness of the `/agent-all-codex`
pipeline and cannot invoke Phase 3 dispatch, Phase 4 gate, or pathspec commit
review. Therefore:

- Do not call `/agent-all-codex` from inside a `codex exec` research prompt —
...
`/agent-all-codex`'s quality gate is enforced in-phase by Phase 4's sequential
...
- If sweep output must feed an `/agent-all-codex` gate, launder it through one
  genuine Phase 4 sequential reviewer dispatch so the audit-token contract still
  fires.

## Non-goals

- Do **not** call `/agent-all-codex` from inside a research sweep (degenerates
...
- Do **not** rebuild `/agent-all-codex` as a second research-sweep "frontend"
