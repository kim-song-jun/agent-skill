...
1. If `mode=create`:
   ...
   - Confirm file does not exist (or `--force` is set). Confirmation via `ask_user`.

2. If `mode=update`:
   - Read existing page content (implicit model read).
   - Confirm overwrite unless `--force` (via `ask_user`).
...
3. Otherwise:
   a. Write `<wikiDir>/<file>` via `apply_patch`.
   b. Update `INDEX.md` via `appendIndexEntry(raw, entry)` and write the updated raw via `apply_patch`.
