...
| Multiple matches | null | [entries] | Present disambiguation list; user picks or refines (via `ask_user`) |
| Tag-only matches | null | [entries] | Present list; user picks or proceeds to Phase B (via `ask_user`) |
| No match | null | [] | Ask user via `ask_user`: "Create new page titled `<query>`?" → Phase B |

## Read Path (match found)

1. Resolve `<wikiDir>/<entry.file>`.
2. Print the full page content (BLUF section first).
3. Ask via `ask_user`: *"Update this page? (y/n)"*
   - Yes → proceed to Phase B with `mode=update`.
   - No → done (exit 0).

## Disambiguation Path

Present up to 5 candidates. User selects one by number or types a refined query (via `ask_user`).

## New Page Path (no match)

Ask via `ask_user`: *"No wiki page for '<query>'. Create one? (y/n)"*
