...
1. Resolve the wiki directory: `$CODEX_PROJECT_DIR/.wiki/` (default). Respect an override from `.agent-all.json` key `wiki.dir` if present.
...
2. Check for `.wiki/INDEX.md`:
   ...
   - **Missing** → prompt user via `ask_user`: *"No wiki found at .wiki/. Create a new wiki scaffold? (y/n)"*
     - Yes → write `.wiki/INDEX.md` from `templates/index.md.tpl` and `.wiki/.gitignore` containing `*.tmp` (use `apply_patch`).
...
5. Hand off to Phase A (`phases/1-route.md`) for read/write operations, or Phase 3 (`phases/3-compile.md`) for `run /wiki compile`.
