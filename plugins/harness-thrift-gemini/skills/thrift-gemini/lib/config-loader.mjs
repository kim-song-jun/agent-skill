// Load .thrift.json with schema validation + DEFAULTS fallback (Gemini port).
//
// Schema differences vs CC:
//   - summariser.model default is "gemini-flash" (not haiku).
//   - cache.vertex sub-block: {minTokenThreshold, storageTimeHours, tier}.
//   - audit.estimateBaseline default is "naive-gemini".
//
// Contract:
...
export const DEFAULTS = {
  ...
  cache: {
    ...
    vertex: {
      ...
      // Vertex context caching requires a minimum prefix size for cache
      // entries to be created. Sub-threshold prime calls pay full cost
      // and produce NO cache entry. Current value approximates the
      // gemini-1.5-pro minimum as of 2026-05; newer models trend higher.
      minTokenThreshold: 32000,
      // TODO verify against current Google Vertex pricing:
      // Vertex bills cached prefixes per storage hour separately from
      // per-read. Typical session: 1 hour. Long-running daemons may
      // bump this; the cost-estimator multiplies storage hours into
      // the prime cost.
      storageTimeHours: 1,
      // "paid" or "free". On "free" the ROI gate refuses to prime.
      tier: "paid",
    },
  },
  contextMode: {
...
function validate(config) {
  ...
  if (c) {
    ...
    if (!isPosInt(c.warmInterval) || c.warmInterval > 290) {
      errors.push({ field: "cache.warmInterval", message: "must be positive integer ≤290 (cache TTL window)" });
    }
    ...
    const v = c.vertex;
    if (v) {
      if (!isPosInt(v.minTokenThreshold)) {
        errors.push({ field: "cache.vertex.minTokenThreshold", message: "must be positive integer (Vertex context cache minimum prefix)" });
      }
      if (!isPosNumber(v.storageTimeHours)) {
        errors.push({ field: "cache.vertex.storageTimeHours", message: "must be positive number" });
      }
      if (!["paid", "free"].includes(v.tier)) {
        errors.push({ field: "cache.vertex.tier", message: "must be 'paid' or 'free'" });
      }
    } else {
      errors.push({ field: "cache.vertex", message: "required (Gemini-specific cache block)" });
    }
  } else {
...
export function loadConfig(path) {
  if (!path || !existsSync(path)) {
    return {
      ...
      warning: ".thrift.json not found; using built-ins. Run /thrift-gemini to seed.",
    };
  }
  let raw;
  try {
