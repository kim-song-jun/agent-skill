...
- **Change history** — see [CHANGELOG.md](CHANGELOG.md). 1676 tests, all green.
...
| Layer | Status | Note |
|---|---|---|
| Unit/integration tests | ✅ **1676/1676 passing** | Mock toolCallers + isolated lib tests; release-doc, policy, Codex hook-schema, task-ledger, Codex exec, release-audit, release-fixture-smoke, command-surface, and visual-qa regressions |
...
| Claude/Codex CLI runtime | ✅ live smoke probe available | `./scripts/release-smoke.sh --fast --with-live-cli` probes installed `claude`/`codex` versions and verifies Codex `exec [PROMPT]` support; release fixture smoke also imports the installed Codex `agent-all-codex` sequential helper |
