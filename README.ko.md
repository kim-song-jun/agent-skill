...
오늘 Claude Code에서 동작, 그리고 **Cursor, GitHub Copilot CLI, VS Code Copilot, Codex CLI, Gemini CLI** 크로스 플랫폼 포트 포함. 19개 플러그인, 7개 슬래시 명령, 하나의 마켓플레이스.

```
/agent-init                                    # 모든 git 저장소 부트스트랩 (Phase A — 프로젝트당 한 번)
/agent-all "OAuth 추가" --loop --qa             # 테스트 + visual-qa 둘 다 통과까지, PR 열기 (Phase C)
/agent-handoff .agent-skill/tasks/T-20260611-001-oauth.md  # handoff + 새 세션 재개 프롬프트 작성
...
고르고 host-level 설치를 확인한 뒤 `/agent-init` 또는 해당 project-local
bootstrap으로 넘어가세요.
...
그림이 포함된 상세 사용설명서는 [docs/USER_MANUAL.ko.md](docs/USER_MANUAL.ko.md)에 있습니다. 전역 플러그인 설치와 프로젝트별 `/agent-init` 차이, init이 다시 필요한 경우, `/agent-all` 요청 예시, `/thrift` 자동 추천, 플랫폼별 실제 지원 범위를 한 문서에서 순서대로 확인할 수 있습니다.

![agent-skill 빠른 시작](docs/assets/user-manual/cards/01-quick-start.png)

핵심만 요약하면:

- **Claude Code 플러그인 설치는 머신/user scope 작업**입니다. 설치 후 Claude Code를 재시작하거나 `/reload-plugins`를 실행합니다.
- **`/agent-init`은 프로젝트별 작업**입니다. 새 git 저장소에서는 한 번 필요하고, 이미 scaffold가 있는 프로젝트는 다시 init하지 않습니다. 예외: v0.7.10 `wiki-capture` PostToolUse hook, v0.7.8 compaction 복원 훅(`session-resume`, `agent-all-continue`), v0.7.7 dirty-tree Edit/Write guard는 `/agent-init`으로 설치됩니다 — 기존 운영 프로젝트는 이를 적용하기 위해 `/agent-init`을 한 번 재실행해야 합니다.
- **일상 작업은 `/agent-all "..." --loop --qa`**로 시작합니다. UI 작업은 `--qa`를 붙이는 것이 기본입니다.
- 세션이 길어지고 큰 출력이 반복되면 context-mode router가 `/thrift` 사용을 추천합니다.
...
`claude` CLI를 통해 Claude Code 필수 6개와 승인된 foundation(`superpowers@claude-plugins-official`, `context-mode@context-mode`)을 한 번에 설치. `--all`로 19개 전체 (CLI-platform sibling 포함), `--cli=codex|copilot|gemini|cursor`로 특정 플랫폼 세트, 또는 `--foundations-only`로 foundation만 부트스트랩. `--dry-run`을 붙이면 `claude` 호출 없이 정확한 계획만 출력합니다.
...
`scripts/update.sh`가 레포를 자동 위치 파악 (또는 temp dir에 clone), 최신 pull, vendored lib 검증 (`sync-lib.mjs --check`) 후, 이미 설치된 선택 플러그인을 강제 업데이트(언인스톨 후 재설치)하고 누락분은 `install-all.sh`로 설치합니다. `--all`로 19개 모두, `--cli=cursor|copilot|codex|gemini`로 한 플랫폼. 전역 CLI config 파일은 패치하지 않음.
...
카운트가 6 미만이면 (Claude Code 권장 세트: builder + floor + thrift + explore + debug + data) 최근 추가분이 빠져 있는 것.
...
dispatch 중에는 변경 파일과 실패 상태를 분류해 역할을 동적으로 고릅니다:
frontend 작업은 design/QA를, data lifecycle 변경은 data review를,
security-sensitive 변경은 security review를 붙이고, 반복 failure signature는
추가 구현자가 아니라 planner/user decision으로 올립니다.

```
/agent-all "Google OAuth 추가"                        # 프롬프트 → PR
/agent-all .agent-skill/tasks/T-20260611-001-oauth.md # 작성된 task 사용
/agent-all "flaky 테스트 수정" --loop --max-iter=5    # 테스트 통과까지 시도
/agent-all "..." --no-pr                             # 로컬 전용 (PR 없음)
```

기본적으로 `--max-iter`, `--max-cost`(기본 $500), 설정된
`--max-runtime-sec`로 제한되고, 완료 여부는 `.agent-all.json`의 break
condition이 결정합니다. `--max-iter=0` 또는 `loop.maxIter: null`이면 반복
횟수는 무제한입니다. 그래도 비용/런타임 예산, hard policy hook, 사용자
중단, 반복 failure signature가 loop를 멈출 수 있습니다.

### `/agent-handoff` — 긴 작업을 깔끔하게 이어받기

진행 중인 task doc, 안전한 git 상태, `.agent-all-state.json`을 읽어
`.agent-skill/handoff/<display-id>-<slug>.handoff.md`와
`.agent-skill/handoff/<display-id>-<slug>.session.md`를 생성합니다. 새 task
doc은 파일명에 `T-YYYYMMDD-NNN` display id를 쓰고, frontmatter/metadata에는
안정적인 `AS-TASK-*` canonical id를 기록해 병렬 실행 간 충돌을 피합니다.
사람이 읽는 상태 요약과 machine-readable metadata가 함께 들어 있어
`/agent-all <task> --resume`이 추천 next action을 자동으로 surface할 수 있습니다.
...
handoff 생성 중에는 `git reset`, reseed 명령, `--apply`,
`docker volume rm` 같은 위험 명령을 실행하지 않습니다. 대신 session prompt에
`사용자 승인 필요`로 표시합니다. non-TTY 모드에서는 추천 resume action을
자동 선택하고 `.agent-skill/runs/handoff-audit.jsonl` 및
`.agent-skill/runs/handoff/interactions.jsonl`에 근거를 기록합니다.

### Artifact Policy

생성되는 control-plane 산출물은 기본적으로 `.agent-skill/` 아래에
저장됩니다. task 문서는 `.agent-skill/tasks/`, spec은
`.agent-skill/specs/`, plan은 `.agent-skill/plans/`, handoff 파일은
`.agent-skill/handoff/`, task registry는 `.agent-skill/registry/tasks.json`,
run log는 `.agent-skill/runs/`, visual QA report는
`.agent-skill/reports/visual-qa/`, debug log는
`.agent-skill/reports/debug/`, thrift audit은
`.agent-skill/reports/thrift/`를 사용하고 baseline은
`.agent-skill/baselines/`에 둡니다. 기존 `docs/tasks/` task
문서는 resume/migration을 위해 계속 읽을 수 있으며, `/agent-init`은 기존
사용자 문서를 자동 삭제하지 않습니다.

`.agent-all.json`에서 `"artifact": {"root": ".custom-agent", "exportDocs": false}`로
root를 바꿀 수 있습니다. `exportDocs: true`는 선택한 report를 publication용
`docs/`로 명시적으로 mirror하는 워크플로에서만 켜세요. 기본 runtime
control-plane 파일은 `docs/` 밖에 둡니다.
...
결과: `.agent-skill/reports/visual-qa/<date-or-slug>/report.md` (+ comprehensive 모드에선 `verdict.json`).
...
Audit report는 기본적으로 `.agent-skill/reports/thrift/audit-<date>.md`에 기록됩니다.
...
Phase 3는 이제 **3a (scoping) → 3b (ask) → 3c (implement)**로 실행됩니다. 각 implementer subagent가 먼저 read-only scoping pass를 수행하고 아키텍처/스펙 모호점 결정들을 구조화된 JSON payload로 반환. coordinator는 이를 공통 `agent-interaction/v1` schema로 정규화한 뒤 Claude에서는 native `AskUserQuestion`, Codex/Copilot/Cursor/Gemini에서는 prompt/markdown surface로 렌더링합니다. 답변이 baked-in된 채로 subagent를 재-dispatch.

**Non-TTY 모드** (야간 루프, `--yes`, iteration ≥ 2)에서는 recommended low/medium-risk 옵션을 자동 선택하고 `.agent-all-state.json`, `.agent-skill/runs/<run-id>/decisions.md`, `.agent-skill/runs/<run-id>/interactions.jsonl`에 로그합니다. High-risk 옵션은 자동 승인하지 않고 block/pause합니다. `/agent-handoff`와 `/agent-all --resume`도 같은 interaction 모델로 resume prompt를 처리합니다.

공유 Node policy engine(`agent-policy-event/v1` ->
`agent-policy-result/v1`) 기반의 `floor-policy` hook으로 강제.
`verification_passed`, reviewer audit token, loop runaway/cost/repeated
failure stop, 위험 shell 명령, pathspec commit, dynamic spawn metadata,
verification adapter lifecycle event, non-TTY decision logging,
control-plane artifact secret/privacy redaction을 검증합니다.
handoff/session prompt, verification evidence, visual/debug/thrift
report, policy/interaction/spawn log, PR body는 저장/공유 전에 scan됩니다.
High severity는 기본 차단, medium severity는 mask됩니다. 결과는
`.agent-skill/runs/<run-id>/policy-log.jsonl`에 append합니다. dynamic spawn은
role, reason, wave, cost estimate를
`.agent-skill/runs/<run-id>/spawn-log.jsonl`에도 append합니다. 사용자/resume
결정은 `.agent-skill/runs/<run-id>/interactions.jsonl`에 append합니다.
Redaction summary는 원문 없이 rule/count metadata만
`.agent-skill/runs/<run-id>/redaction-audit.jsonl`에 남깁니다. 프로젝트별
opt-out/allowlist는 값이 아니라 path/rule 단위로 `.agent-all.json`에 둡니다:
...
`docs/superpowers/specs/2026-06-11-policy-hook-engine.md`,
`docs/superpowers/specs/2026-06-11-dynamic-agent-orchestration.md`.
...
| CLI/API/data/SQL 완료 기준 | `--break-condition='{"type":"verification-adapter",...}'` | adapter evidence → `.agent-skill/runs/<run-id>/verification-evidence.jsonl` |
| 커스텀 명령 | `--break-condition='make ci'` | 한 줄 명령 |
| 명시적 spec | `--break-condition='{"type":"composite","steps":[...]}'` | JSON spec |

프로젝트에서 **첫 번째** `/agent-all --loop` 실행 시 Phase 0이 대화형(test / visual-qa / verification adapter / custom / composite)으로 물어보고 `.agent-all.json`에 저장 여부를 묻습니다. 이후 실행은 저장된 값 재사용. `--reconfigure`로 재프롬프트 강제; `--yes` / non-TTY는 프롬프트 건너뜀.

Verification adapter는 기존 web UI 중심 완료 체크를 일반화합니다. `/visual-qa`는 제거하지 않고 `verify:web-ui` wrapper로 evidence를 남깁니다.

```bash
/agent-all "CLI 출력 고정" --loop \
  --break-condition='{"type":"verification-adapter","adapter":"cli","config":{"command":"my-tool --help","goldenStdoutPath":"test/golden/help.txt"}}'

/agent-all "API schema 검증" --loop \
...
/agent-all "warehouse query 검증" --loop \
...
기본 adapter는 `verify:web-ui`, `verify:cli`, `verify:api-contract`, `verify:notebook-data`, `verify:sql-db`, `verify:batch-job`입니다. 모든 adapter 결과는 `adapter`, `status`, 선택적 `command`, `artifacts`, `summary`, `failures`, 재현성 metadata를 담은 `verification-evidence/v1`로 기록됩니다.

데이터 작업은 `harness-data`를 설치하고 `/data-runner`로 task doc 형태를 먼저 잡은 뒤 `/agent-all`을 실행합니다. task template에는 Dataset / Source, Data Snapshot, Assumptions, Reproducibility, Validation Queries, Artifacts, Data Risks, Rollback / Cleanup용 Data Task Addendum이 포함됩니다. 파괴적 SQL/data 작업은 기본 차단되며, 명시적 사용자 승인 후에만 `allowDestructive=true`를 설정합니다.
...
- **`--max-iter`** 안전 캡 (`0`이면 무제한) + **`--max-cost=USD`** 및 **`--max-runtime-sec=초`** 예산 중단
- **Cost telemetry**: `agent-cost-telemetry/v1` 요약을 `.agent-skill/runs/<run-id>/cost-telemetry.jsonl`에 기록하고 최신 합계를 `state.costTelemetry.summary`에 미러링합니다. 예산 80%에서 사용자 확인을 요구하고 100%에서 중단합니다. 플랫폼이 보고한 cost를 우선 사용하며, token/char 기반 추정은 best-effort이고 `.agent-all.json: telemetry.cost`에서 조정합니다.
...
| `--max-iter=N` | config | 선택적 iteration 안전 캡; `0`이면 무제한 |
| `--max-cost=USD` | 500 | 누적 API 비용 캡; 매 wave 후 체크 |
| `--max-runtime-sec=초` | config | 긴 loop 실행의 선택적 wall-clock 캡 |
...
| `.agent-all.json: breakCondition` | `npm test` (자동 감지) | 저장 spec. 문자열 = shell; 객체 = `shell` / `test-auto` / `visual-qa` / `verification-adapter` / `composite`. |
| `.agent-all.json: stableIters` | 1 | loop 깔끔히 종료 전 필요한 연속 pass 수. |
| `.agent-all.json: telemetry.cost.warnAtRatio` | 0.8 | hard stop 전 예산 경고 비율 |
| `.agent-all.json: telemetry.cost.modelRates` | `{}` | 추정용 프로젝트 소유 token rate override |
...
| iter 1이 "통과"인데 UI가 명백히 깨져있음 | first-run 정책이 `report` (기본) — loop 통과지만 이슈는 report에 기록됨. `.agent-skill/reports/visual-qa/loop-iter-1/report.md` 읽기 | 이슈 수정 후 iter 2가 iter-1 baseline 대비 검증 |
...
Claude Code는 native 마켓플레이스를 가짐 (`/plugin install`)이고, Claude 안에서는 `/agent-init`이 기본 setup 경로입니다. Codex CLI 0.140.0+도 Codex plugin bundle용 native plugin manager(`codex plugin marketplace` + `codex plugin add`)를 제공합니다. 터미널에서 project bootstrap을 하고 싶을 때는 `install-platform.sh --platform=claude|codex`가 같은 project-local 렌더러를 채팅 표면 밖에서 실행합니다. Cursor, GitHub Copilot, Gemini CLI, VS Code Copilot은 아직 이 워크플로용 비교 가능한 플러그인 마켓플레이스가 없으므로 같은 wrapper가 프로젝트에 올바른 파일을 씁니다. CLI 플랫폼은 그 도구가 기대하는 레이아웃의 config/hook/skill 파일을 받고, VS Code Copilot은 에디터 지침 전용 파일만 받음.
...
기본값은 operational scaffold 설치. Claude가 아닌 플랫폼은 사용 가능한 무거운 테마를 기본으로 설치합니다: 공통으로 builder + floor + thrift, Codex는 debug까지 포함. 하나만 설치하려면 `--theme=floor`, `--theme=thrift`, 또는 Codex 전용 `--theme=debug`를 사용하세요. Claude project bootstrap은 기본 floor-enabled operational scaffold, `.visual-qa.json`/`.agent-all.json` 없이 무거운 builder scaffold만 쓰는 `--theme=builder`, 그리고 `--lite`를 지원하며 Claude slash-command 플러그인 설치는 `install-all.sh` 또는 `/plugin install`로 진행합니다. `--lite`는 builder-only 경량 scaffold이며, Codex에서는 `codex-init --lite`로 전달되어 floor/thrift/debug 파일과 전역 config 스니펫을 건너뜀. VS Code Copilot은 지침 전용 경로라 floor/thrift/debug theme 설치를 받지 않음. `--lang=ko|en|auto`로 생성된 루트 지침을 맞추고, floor config가 설치될 때는 `.agent-all.json`의 language 값도 함께 맞출 수 있음. Claude/Codex operational 설치는 가능할 때 승인된 foundation 플러그인(`superpowers@claude-plugins-official`, `context-mode@context-mode`)만 자동 갱신하고, `claude` CLI가 없거나 승인된 foundation 갱신이 실패하면 degraded foundation 경고를 출력한 뒤 scaffold를 계속 진행합니다. Lite 설치는 기본 자동 foundation 갱신을 건너뛰며, 무거운 산출물 없이 strict foundation 갱신만 원할 때 `--lite --update-foundations`를 명시하세요. `--update-foundations`는 갱신 실패를 strict 실패로 만들 때, `--no-update-foundations`는 opt-out할 때, `--dry-run`은 `claude` 호출 없이 승인된 계획만 볼 때 사용하세요. Claude와 Codex `all`, `builder`, `--lite`, 그리고 Codex `--theme=debug` 설치는 post-install doctor를 자동 실행하며, 검증을 의도적으로 미룰 때만 `--no-doctor`를 넘기세요. `install-platform.sh`는 project-local 파일을 쓰고 전역 config 스니펫을 stdout으로 출력할 뿐, 전역 CLI config 파일을 패치하지 않음.
...
| 플랫폼 | 작성되는 파일 | 비고 |
|---|---|---|
| **Claude Code** | `CLAUDE.md`, `AGENTS.md`, `.claude/agents/*.md`, `.claude/hooks/*.mjs`, `.visual-qa.json`, `.agent-all.json` | `/agent-init`과 같은 operational scaffold를 터미널에서 project bootstrap; `--theme=builder`는 무거운 builder artifact는 유지하고 floor config는 생략. marketplace 플러그인 설치는 여전히 `install-all.sh` 또는 `/plugin install`. |
...
Codex user-scope plugin bundle은 marketplace snapshot을 갱신하고 Codex
플러그인을 강제로 다시 add한 뒤 enabled 상태를 검증합니다.
...
❌ codex plugins install harness-floor-codex   # 복수 `plugins` + `install`은 Codex CLI 표면이 아님
❌ gemini extensions install harness-floor-gemini
```

Codex는 `./scripts/update-codex-plugins.sh` 또는 단수형
`codex plugin add ...` 표면을 사용하세요. Copilot과 Gemini는
`./scripts/install-platform.sh`를 사용합니다.
...
Claude cleanup은 생성 sentinel 섹션과 생성 hook/agent/settings 등록을 걷어내며, sentinel 없는 루트 `CLAUDE.md`/`AGENTS.md` 가이드는 `--force-root-clean`이 명시된 경우에만 제거합니다. Codex cleanup은 생성된 `.codex/skills`, `.codex/hooks`, floor/thrift config 파일, debug skill 디렉터리, task template, helper script를 제거합니다. Debug 증거인 `.agent-skill/reports/debug/`와 `.debug-artifacts/`는 보존합니다. 기본값은 agent-skill sentinel이 없는 루트 `AGENTS.md`를 보존하고, `--force-root-clean`을 넘기면 생성된 것으로 보이는 루트 가이드까지 제거합니다. Cursor, Copilot, Gemini, VS Code Copilot은 당분간 plugin-specific cleanup 또는 수동 검토를 사용합니다.
...
1. **Loop 중단 조건**: break condition 통과, 선택적 `--max-iter`, `--max-cost` (기본 $500), `--max-runtime-sec`, hard policy hook, 사용자 중단, 반복 failure signature.
...
조합: 깨진 코드 sneak through 못 함, 비용 폭발 못 함, 무제한 반복 모드에서도 정책/반복 실패 중단이 작동함.
...
- **프로젝트 계획** — vision, workstream, artifact policy, canonical id, metrics, release policy, issue taxonomy는 [PROJECT_PLAN.md](PROJECT_PLAN.md) 참조.
- **로드맵** — milestone 순서와 active issue mapping은 [ROADMAP.md](ROADMAP.md) 참조.
- **Cross-platform support matrix** — [SUPPORT_MATRIX.md](SUPPORT_MATRIX.md) 참조. `plugins/harness-core/capabilities/catalog.mjs`에서 생성됩니다.
- **아키텍처 인덱스** — 날짜별 spec/plan 정리표는 [docs/architecture/README.md](docs/architecture/README.md) 참조.
...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 2352 tests, 모두 통과.
...
- **CLI 런타임 검증 체크리스트** — [docs/superpowers/specs/2026-05-18-cli-runtime-verification-checklist.md](docs/superpowers/specs/2026-05-18-cli-runtime-verification-checklist.md) 참조.
...
버전: `harness-builder` `v0.7.10`, `harness-floor` `v0.7.10`, 나머지 설치 가능한 `agent-skill` 플러그인 17개도 같은 release train의 `v0.7.10`로 배포합니다. 내부 artifact/config schema version은 별도의 호환성 계약으로 유지됩니다.
...
- `node scripts/release-audit.mjs`, `node scripts/github-governance-check.mjs`, `node scripts/docs-structure-check.mjs`, `node scripts/release-provenance.mjs --release=<rc-tag> --out-dir=.agent-skill/releases/<rc-tag>`, `node scripts/release-fixture-smoke.mjs`, `node scripts/skill-eval.mjs --smoke --no-write --json`, `./scripts/release-smoke.sh --fast --with-live-cli`, `node scripts/release-publish-preflight.mjs --base=origin/main`, `node --test`, `node scripts/sync-lib.mjs --check`, `node scripts/generate-support-matrix.mjs --check`가 모두 통과해야 함.
- 같은 commit에서 public PR smoke와 local release gate가 GitHub workflow/template/docs governance, release-candidate evidence, release-audit, fresh release fixture, live Claude/Codex smoke, target-project smoke, 전체 Node test suite, vendored-lib sync, support matrix drift check를 모두 통과해야 함.
- branch/tag push 전에 `node scripts/release-publish-preflight.mjs --base=origin/main`이 통과해야 함. Workflow 변경은 GitHub CLI `workflow` scope가 필요하며, 공개 PR smoke CI는 local release gate를 대체하지 않음.
- 각 rollout 대상 프로젝트에서 `node scripts/target-project-smoke.mjs --target=/path/to/target --platform=claude,codex --lang=ko`가 통과해야 함. doctor가 실패하면 권장 `install-platform.sh --force` 명령으로 갱신 후 다시 smoke 실행.
...
- release manifest는 checkout commit, marketplace checksum, 플러그인 디렉터리 checksum, vendored-lib/template aggregate checksum, signed-tag 상태를 기록합니다. Signed tag는 release process가 `--require-signed-tag`를 선택하기 전까지 warning으로 보고됩니다.
...
- **Non-TTY auto-pick은 틀릴 수 있음.** 야간 루프는 low/medium-risk 결정을 subagent의 `recommended_index`로 자동 해결. 잘못된 선택은 다음날에야 드러남. 모든 auto-pick은 `.agent-skill/runs/<run-id>/decisions.md`와 `.agent-skill/runs/<run-id>/interactions.jsonl`에 reasoning과 함께 기록됩니다. High-risk 선택지는 block/pause하고 사용자 입력을 요구합니다.
...
[ROADMAP.md](ROADMAP.md)에 milestone map을 유지합니다. 다음 계획 축은
capability core/platform adapter, verification/data 확장, policy hook과
dynamic orchestration, cost/eval telemetry, public CI governance,
supply-chain provenance입니다.
...
./scripts/release-smoke.sh --fast --with-live-cli  # 설치된 Claude/Codex CLI와 명령 표면도 probe
node scripts/release-audit.mjs           # Claude/Codex 릴리즈 준비 상태 매트릭스
node scripts/github-governance-check.mjs # 공개 workflow, template, label, governance 문서
node scripts/docs-structure-check.mjs    # 필수 문서와 local Markdown link 확인
...
node --test                              # 2352/2352 통과 필수
node scripts/sync-lib.mjs --check        # vendored shared libs 동기화 확인
node scripts/generate-support-matrix.mjs --check
```

릴리즈 artifact를 소비할 때는 `scripts/install-all.sh` 또는 `scripts/install-platform.sh`에 `--verify-checksums --manifest=/path/to/release-manifest.json`을 넘기고, `scripts/update.sh`에는 `--verify-provenance --manifest=/path/to/release-manifest.json`을 넘겨 checksum을 다시 확인합니다.
