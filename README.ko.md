...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 1279+ tests, 모두 통과.
...
| 레이어 | 상태 | 비고 |
|---|---|---|
| Unit/integration 테스트 | ✅ **1279/1279 통과** | Mock toolCaller + 격리된 lib 테스트; decision-surfacing용 +33 |
...
- **Cursor / Gemini / VS Code Copilot은 decision-surfacing soft 강제만 가능.** 이 플랫폼들은 tool-call hook이 없어서 protocol이 rules / `GEMINI.md` / `copilot-instructions.md`에 prompt로만 주입됩니다. 비준수 subagent를 harness 레이어에서 차단할 수 없습니다. Claude Code / Copilot CLI / Codex CLI (수동 `[[hooks.agent]]` merge 후)는 hook 기반 hard enforce.
