...
오늘 Claude Code에서 동작, 그리고 **Cursor, GitHub Copilot CLI, VS Code Copilot, Codex CLI, Gemini CLI** 크로스 플랫폼 포트 포함. 19개 플러그인, 6개 슬래시 명령, 하나의 마켓플레이스.
...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 1983 tests, 모두 통과.
...
| 레이어 | 상태 | 비고 |
|---|---|---|
| Unit/integration 테스트 | ✅ **1983/1983 통과** | Mock toolCaller + 격리된 lib 테스트; release-doc, policy, Codex hook-schema, task-ledger, Codex exec, release-audit, release-candidate evidence, provenance manifest, public GitHub governance, docs structure, release publish preflight, target-project smoke, skill-eval, command-surface, doctor, cleanup, visual-qa 회귀 포함 |
...
버전: `harness-builder` `v0.6.0`, `harness-floor` `v0.6.0`, 나머지 설치 가능한 `agent-skill` 플러그인 17개도 같은 release train의 `v0.6.0`으로 배포합니다. 내부 artifact/config schema version은 별도의 호환성 계약으로 유지됩니다.
