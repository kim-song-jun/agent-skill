...
- **`/agent-init`은 프로젝트별 작업**입니다. 새 git 저장소에서는 한 번 필요하고, 이미 scaffold가 있는 프로젝트는 다시 init하지 않습니다. 예외: v0.7.9 compaction 복원 훅(`session-resume`, `agent-all-continue`)과 v0.7.7 dirty-tree Edit/Write guard는 `/agent-init`으로 설치됩니다 — 기존 운영 프로젝트는 이를 적용하기 위해 `/agent-init`을 한 번 재실행해야 합니다.
...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 2332 tests, 모두 통과.
...
버전: `harness-builder` `v0.7.9`, `harness-floor` `v0.7.9`, 나머지 설치 가능한 `agent-skill` 플러그인 17개도 같은 release train의 `v0.7.9`로 배포합니다. 내부 artifact/config schema version은 별도의 호환성 계약으로 유지됩니다.
