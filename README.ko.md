...
| Claude/Codex CLI 런타임 | ✅ live smoke probe 가능 | `./scripts/release-smoke.sh --fast --with-live-cli`이 설치된 `claude`/`codex` 버전과 Codex `exec [PROMPT]` 지원을 probe함; release fixture smoke도 Claude 터미널 `install-platform.sh --platform=claude` operational 및 `--lite` 경로를 검증하고, 설치된 Codex `agent-all-codex` 및 `visual-qa-codex` sequential helper를 import하며 Codex debug-only fixture를 검증 |
