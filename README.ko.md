...
Claude 또는 Codex 프로젝트 설치 후 `node /path/to/agent-skill/scripts/doctor.mjs --target=/path/to/my-project --platform=claude|codex`로 수동 재검증할 수 있습니다. project-local scaffold를 검증하고, 가능하면 operational/builder/lite profile을 자동 감지하며, 누락 artifact는 non-zero exit로 보고하고 승인된 foundation 누락은 경고합니다.
...
# - .github/copilot-instructions.md만 (VS Code Copilot)
```
...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 1697 tests, 모두 통과.
...
| 레이어 | 상태 | 비고 |
|---|---|---|
| Unit/integration 테스트 | ✅ **1697/1697 통과** | Mock toolCaller + 격리된 lib 테스트; release-doc, policy, Codex hook-schema, task-ledger, Codex exec, release-audit, release-fixture-smoke, command-surface, doctor, visual-qa 회귀 포함 |
