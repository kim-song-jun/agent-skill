...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 1981 tests, 모두 통과.
...
| 레이어 | 상태 | 비고 |
|---|---|---|
| Unit/integration 테스트 | ✅ **1981/1981 통과** | Mock toolCaller + 격리된 lib 테스트; release-doc, policy, Codex hook-schema, task-ledger, Codex exec, release-audit, release-candidate evidence, provenance manifest, public GitHub governance, docs structure, release publish preflight, target-project smoke, skill-eval, command-surface, doctor, cleanup, visual-qa 회귀 포함 |
...
버전: `harness-builder` `v0.3.0`, `harness-floor` `v0.5.1` (visual-qa 런타임 wiring + agent-init i18n patch), Claude `harness-thrift`/`harness-explore`/`harness-debug`/`harness-data` `v0.1.0`, Codex 및 기타 플랫폼 포트 `v0.1.0`.
