...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 1755 tests, 모두 통과.
...
| 레이어 | 상태 | 비고 |
|---|---|---|
| Unit/integration 테스트 | ✅ **1755/1755 통과** | Mock toolCaller + 격리된 lib 테스트; release-doc, policy, Codex hook-schema, task-ledger, Codex exec, release-audit, release-fixture-smoke, command-surface, doctor, cleanup, visual-qa 회귀 포함 |
...
| Claude/Codex CLI 런타임 | ✅ live smoke probe 가능 | `./scripts/release-smoke.sh --fast --with-live-cli`이 설치된 `claude`/`codex` 버전과 Codex `exec [PROMPT]` 지원을 probe함; release fixture smoke도 Claude 터미널 `install-platform.sh --platform=claude` operational 및 `--lite` 경로, Codex operational/lite/builder/floor/thrift/debug fresh install, Claude/Codex install→uninstall roundtrip, 설치된 Codex `agent-all-codex` 및 `visual-qa-codex` sequential helper, Codex debug-only fixture를 검증 |
