...
Claude cleanup은 생성 sentinel 섹션과 생성 hook/agent/settings 등록을 걷어내며, sentinel 없는 루트 `CLAUDE.md`/`AGENTS.md` 가이드는 `--force-root-clean`이 명시된 경우에만 제거합니다. Codex cleanup은 생성된 `.codex/skills`, `.codex/hooks`, floor/thrift config 파일, debug skill 디렉터리, task template, helper script를 제거합니다. Debug 증거인 `docs/debug/`와 `.debug-artifacts/`는 보존합니다. 기본값은 agent-skill sentinel이 없는 루트 `AGENTS.md`를 보존하고, `--force-root-clean`을 넘기면 생성된 것으로 보이는 루트 가이드까지 제거합니다. Cursor, Copilot, Gemini, VS Code Copilot은 당분간 plugin-specific cleanup 또는 수동 검토를 사용합니다.
...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 1776 tests, 모두 통과.
...
| 레이어 | 상태 | 비고 |
|---|---|---|
| Unit/integration 테스트 | ✅ **1776/1776 통과** | Mock toolCaller + 격리된 lib 테스트; release-doc, policy, Codex hook-schema, task-ledger, Codex exec, release-audit, release-candidate evidence, release publish preflight, target-project smoke, release-fixture-smoke, command-surface, doctor, cleanup, visual-qa 회귀 포함 |
...
버전: `harness-builder` `v0.3.0`, `harness-floor` `v0.5.1` (visual-qa 런타임 wiring + agent-init i18n patch), Claude `harness-thrift`/`harness-explore`/`harness-debug` `v0.1.0`, Codex 및 기타 플랫폼 포트 `v0.1.0`.
...
- `node scripts/release-audit.mjs`, `node scripts/release-fixture-smoke.mjs`, `./scripts/release-smoke.sh --fast --with-live-cli`, `node scripts/release-publish-preflight.mjs --base=origin/main`, `node --test`, `node scripts/sync-lib.mjs --check`가 모두 통과해야 함.
- 같은 commit에서 local-only deployment gate가 release-candidate evidence, release-audit, fresh release fixture, live Claude/Codex smoke, POSCO target smoke, 전체 Node test suite, vendored-lib sync를 모두 통과해야 함.
- branch/tag push 전에 `node scripts/release-publish-preflight.mjs --base=origin/main`이 통과해야 함. 이 브랜치는 workflow 파일을 포함하지 않으므로 GitHub CLI `workflow` scope가 필요 없음.
