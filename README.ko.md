...
- **`/agent-init`은 프로젝트별 작업**입니다. 새 git 저장소에서는 한 번 필요하고, 이미 scaffold가 있는 프로젝트는 다시 init하지 않습니다.
...
- **변경 히스토리** — [CHANGELOG.md](CHANGELOG.md) 참조. 2006 tests, 모두 통과.
...
| 레이어 | 상태 | 비고 |
|---|---|---|
| Unit/integration 테스트 | ✅ **2006/2006 통과** | Mock toolCaller + 격리된 lib 테스트; release-doc, policy, Codex hook-schema, native Codex plugin updater, task-ledger, Codex exec, release-audit, release-candidate evidence, provenance manifest, public GitHub governance, docs structure, release publish preflight, target-project smoke, skill-eval, command-surface, doctor, cleanup, visual-qa 회귀 포함 |
...
버전: `harness-builder` `v0.6.11`, `harness-floor` `v0.6.11`, 나머지 설치 가능한 `agent-skill` 플러그인 17개도 같은 release train의 `v0.6.11`으로 배포합니다. 내부 artifact/config schema version은 별도의 호환성 계약으로 유지됩니다.
