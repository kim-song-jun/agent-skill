...
### `/harness-view` 폴리시 — 전체 높이 레이아웃 + 검색 개선

v0.7.19 대시보드 후속 폴리시. 레이아웃이 헤더 높이를 더는 하드코딩하지 않습니다: `calc(100vh - 52px)`를 flexbox 컬럼으로 교체해 하단의 작은 gap을 없애고, 좁은 화면에서 사이드바를 fixed 오버레이가 아닌 in-flow 접이식 블록으로 바꿔 매직 헤더-오프셋 의존을 제거했습니다. 사이드바 검색은 이제 쿼리가 행을 걸러내면 섹션 라벨과 "None" 빈 placeholder도 숨기고, 키 입력마다 셀렉터를 만드는 대신 사전 구성한 id→pane 맵으로 패널을 찾습니다. 렌더링·딥링크·TOC 동작 변화 없음.
...
스펙, 플랜, 태스크, 손으로 작성한 프로젝트 문서가 더 이상 `.wiki/` 지식 베이스를 우회하지 않습니다. 새로운 `/wiki import` 엔진이 **참조 + 합성, 복사 없이** 문서를 기록합니다 — 각 문서는 한 줄 BLUF, 저비용 모델 합성본, 원본 문서를 가리키는 `sources:` 링크를 가진 토픽 페이지가 됩니다(기존 페이지와 병합 포함).

- **`/wiki import <doc>`** — 문서 하나를 위키로 라우팅합니다: `deriveTopic`이 파일명을 병합 키 슬러그로 정규화하여(한 기능의 스펙, 플랜, 태스크가 하나의 토픽 페이지로 합쳐짐), `wiki.model` 저비용 스크라이브가 복사 금지 가드레일을 명시하며 페이지를 합성합니다.
- **`/wiki import --all`** — 설정된 소스 루트를 소급 임포트합니다: 드라이런 미리보기(docs → 고유 토픽 + 예상 비용)가 먼저 실행되고, `--apply`는 오래된 것부터 임포트하며, `wiki.maxImportUSD` 비용 상한이 실행 범위를 제한합니다. 소스 루트는 프로젝트별로 설정 가능합니다(`.agent-all.json`의 `wiki.sources`/`wiki.exclude`), 첫 실행 시 대화형으로 선택합니다.
- **`wiki-capture.mjs`** — 소스 루트 아래에 문서가 작성되면 `/wiki import`를 비차단 방식으로 알려주는 PostToolUse hook입니다. `/agent-all` 실행 중에는 억제됩니다(위키 자체에 기록하므로).
- **agent-all Phase 2**가 이제 브레인스토밍 스펙을 플랜과 함께 위키 소스로 기록합니다.

참조-중복금지는 스크라이브 프롬프트 + ≤200단어 합성 상한으로 강제되며, 하드 코드 캡은 아닙니다. 기존 운영 프로젝트는 새 `wiki-capture` hook을 적용하기 위해 `/agent-init`을 한 번 재실행해야 합니다.

## Agent-skill v0.7.9 — 2026-06-23

### 컴팩션 복원 지시문에 위키 포인터 추가

v0.7.8을 기반으로 합니다. `session-resume.mjs` SessionStart 훅이 이제 Phase 2에서 페이지를 기록한 경우(`state.wikiPage`) 컴팩션 후 지시문에 한 줄짜리 위키 포인터 — `Relevant wiki: .wiki/<slug>.md` — 를 추가합니다. 컴팩션 이후 재정향된 오케스트레이터는 두 개 대신 세 개의 내구성 있는 앵커를 얻습니다: *위치*(`.agent-all-state.json` 좌표), *내용*(다음 phase 파일), 그리고 이제 *이유*(위키 페이지에 기록된 계획, 결정, 근거). 포인터로만 남아 컨텍스트 덤프가 아니므로 — 지시문은 약 700자를 유지하며 윈도우 압박에 기여하지 않습니다. 위키 auto-loop가 꺼져 있거나 페이지가 아직 기록되지 않은 경우 생략됩니다.
